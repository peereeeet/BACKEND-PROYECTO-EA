...
export const getUserNotificaciones = async (req: Request, res: Response) => {
  try {
    ...
    const notificaciones = await notificacionService.getUserNotificaciones(userId, limit);
...
export const getUnreadNotificaciones = async (req: Request, res: Response) => {
  try {
    ...
    const notificaciones = await notificacionService.getUnreadNotificaciones(userId);
...
export const markAsRead = async (req: Request, res: Response) => {
  try {
    ...
    if (success) {
      return res.status(200).json({ ok: true, message: 'Notificación marcada como leída' });
    } else {
...
export const markAllAsRead = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(200).json({ ok: true, message: `${count} notificaciones marcadas como leídas` });
  } catch (error) {
...
export const deleteNotificacion = async (req: Request, res: Response) => {
  try {
    ...
    const success = await notificacionService.deleteNotificacion(notificacionId);
    
    if (success) {
      return res.status(200).json({ ok: true, message: 'Notificación eliminada' });
    } else {
      return res.status(404).json({ ok: false, message: 'Notificación no encontrada' });
    }
  } catch (error) {

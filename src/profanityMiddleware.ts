...
export const profanityMiddleware = (fieldsToCheck: string[]) => {
  return (req: Request, res: Response, next: NextFunction) => {
    try {
      const lang = (req.headers['accept-language']?.split(',')[0]?.split('-')[0] as any) || 'es';
      
      const result = ProfanityFilter.checkObject(req.body, fieldsToCheck);
      
      if (!result.isClean) {
        logger.warn(`🚫 Contenido bloqueado - Palabras encontradas: ${result.foundWords.join(', ')}`);
        logger.warn(`Usuario: ${(req as any).user?.id || 'Anónimo'} | IP: ${req.ip}`);
...
export const validateMessageContent = profanityMiddleware(['content', 'message', 'texto']);
